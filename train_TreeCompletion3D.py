...
def train():
    ...
    if args.dataset == 'TreeCompletion3D':
        dataset = TreeCompletion3D(args.TreeCompletion3Dpath, prefix="Train")
        dataset_test = TreeCompletion3D(args.TreeCompletion3Dpath, prefix="Validation")
    else:
    ...
    if args.load_model:
        ...
        print("%s's previous weights loaded." % args.model_name)
    ...
    for epoch in range(args.start_epoch, args.nepoch):
        ...
        for i, data in enumerate(tqdm(dataloader, desc="Training Progress"), 0):
            ...
            out2, loss3, loss2, loss1, net_loss = net(inputs, gt)
            ...
            if i % args.step_interval_to_print == 0:
                logging.info(exp_name + ' train [%d: %d/%d]  loss_type: %s, fine2_loss: %f fine1_loss: %f coarse_loss: %f total_loss: %f lr: %f' %
                             (epoch, i, len(dataset) / args.batch_size, args.loss, loss3.mean().item(), loss2.mean().item(), loss1.mean().item(), net_loss.mean().item(), lr))
                print(exp_name + ' train [%d: %d/%d]  loss_type: %s, fine2_loss: %f fine1_loss: %f coarse_loss: %f total_loss: %f lr: %f' %
                             (epoch, i, len(dataset) / args.batch_size, args.loss, loss3.mean().item(), loss2.mean().item(), loss1.mean().item(), net_loss.mean().item(), lr))
    ...
    with torch.no_grad():
        ...
        for loss_type, (curr_best_epoch, curr_best_loss) in best_epoch_losses.items():
            if (val_loss_meters[loss_type].avg < curr_best_loss and loss_type != 'f1') or \
                ...
                save_model('%s/best_%s_network%s.pth' % (log_dir, loss_type, val_loss_meters[loss_type].avg), net)
                logging.info('Best %s net saved!' % loss_type)
                print('Best %s net saved!' % loss_type)
